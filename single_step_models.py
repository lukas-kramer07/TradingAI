...
def train_and_test(model, window, model_name, patience=15, retrain = RETRAIN):
  if model_name not in os.listdir('Training/Models/single') or retrain:
    HISTORY[model_name] = compile_and_fit(model, window, patience)
    model.save(f'Training/Models/single/{model_name}')
  else:
     model = tf.keras.models.load_model(f'Training/Models/single/{model_name}')
